...
10. **감성점수를 특징으로 편입 (배관만, 검증은 보류)** — `recommendations.sentimentScore`를
    ...
    도입 이전이라 오염 여부를 알 수 없는 과거 레코드도 함께 제외). 최소 표본 기준(500건)에
    자동 게이트를 걸어뒀는데, 현재 유효 표본이 495건으로 근소하게 미달해 이번 회차는
    "검증 보류"로 자동 스킵됨(파이프라인 전체가 죽지 않고 이 특징만 건너뜀) — 하루 평균
    약 55건씩 쌓이고 있어 하루 이내 500건을 넘길 것으로 예상, `python-collector/train_model.py`를
    그대로 재실행하면 자동으로 재검증된다.
