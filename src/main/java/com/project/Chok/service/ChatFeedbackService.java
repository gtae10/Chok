...
public class ChatFeedbackService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            - 상승확률: 두 가지 출처 중 하나이며 화면에 뱃지로 항상 구분 표시됩니다.
              "학습 기반"(MODEL) - 과거 데이터로 학습된 로지스틱 회귀 모델의 결과이며,
              "N영업일 기준"이라는 구체적 예측 기간이 함께 표시됩니다(예: 50영업일 기준).
              "추정치"(HEURISTIC) - 학습된 모델이 없거나 검증을 통과하지 못했을 때, 기술
              지표 점수들을 조합한 추정치로 대체된 것이며, 특정 예측기간을 주장하지
              않습니다. 이 둘의 차이(통계적으로 검증된 모델 vs 단순 지표 조합 추정)를
              명확히 구분해서 설명해야 합니다.
            - 하락확률: 별도로 계산하지 않고 "100 - 상승확률"로 표시되는, 상승확률의
              반대쪽 표현입니다.
            - 유력 구간 / 유력 하락구간: 여러 예측기간(3~100영업일)별 모델 중 실제
              holdout AUC가 일정 기준(0.55) 이상인 기간만 후보로 놓고, 그중 상승확률이
              가장 높은(또는 하락 쪽은 가장 낮은) 기간을 보여주는 부가 정보입니다. 기준을
              통과하는 기간이 하나도 없으면 아예 표시되지 않습니다 - 화면에 안 보인다면
              "지금은 통계적으로 뚜렷한 기간이 없다"는 뜻입니다.
